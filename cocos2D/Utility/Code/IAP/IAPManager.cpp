...
namespace Utility
{
	namespace IAP
	{
		...
		void IAPManager::OnInitialize()
		{
            ...
            //
            if(!p_ThreadCheckFulfillment)
            {
                p_ThreadCheckFulfillment = RKUtils::RKThread_Create("thread_init_fullfilment", &IAPManager::checkFulfillment_thread, (void*)this, RKThreadPriority::RKThreadPriority_Normal);
                RKThread_Start(p_ThreadCheckFulfillment);
            }
		}
        ...
        uint32 IAPManager::checkFulfillment_thread(void * data)
        {
            IAPManager * iap = static_cast<IAPManager *>(data);
            if(iap)
            {
                iap->CheckItemFulfillmentAtInit();
            }
			return 0;
        }
		...
		void IAPManager::CheckItemFulfillmentAtInit()
		{
			...
			if (keyGain != "")
			{
				auto listStr = keyGain.Split("#");
				int cur_coin = GetSave->GetDataSave()->GetDataByName(SAVE_STR_COIN);
				for (int i = 0; i < listStr.Size(); i++)
				{
					RKString name_pack = listStr.GetAt(i);
					if (p_listIAPItem.size() > 0 && p_listIAPItem.find(name_pack) != p_listIAPItem.end())
					{
						cur_coin = cur_coin + p_listIAPItem.at(name_pack)->p_GainValue;
						GetSave->GetDataSave()->SetDataAtName(SAVE_STR_COIN, cur_coin);
					}
			...
			{
				p_IsRemoveAds = (GetSave->GetDataSave()->GetcharByName(SAVE_STR_REMOVE_ADS) == "1");
			}
        ...
        void IAPManager::DestroyAllThread()
        {
            if(p_ThreadCheckFulfillment)
            {
                RKUtils::RKThread_WaitForExit(p_ThreadCheckFulfillment);
                RKThread_Destroy(&p_ThreadCheckFulfillment);
            }
