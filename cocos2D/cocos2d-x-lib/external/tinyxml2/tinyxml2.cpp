...
XMLError XMLDocument::LoadFileArk(std::string filename)
{
	RKFile * fp = RKFile_Open(filename.c_str(), RKFileAccessMode::RKFM_READ);
    if(!fp)
    {
	...
	size_t read = RKFile_Read(fp, _charBuffer, size);
	if (read != size) {
